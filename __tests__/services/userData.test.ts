...
const mockDoc = jest.fn((_, ...pathSegments) => ({ path: pathSegments.join('/') }));
...
describe('createUserProfile', () => {
    ...
    it('rethrows errors from setDoc', async () => {
        ...
        await expect(createUserProfile('user-1', {})).rejects.toThrow('Firestore write failed');
    });
});
...
describe('updateUserProfile', () => {
    ...
    it('rethrows errors from updateDoc', async () => {
        ...
        await expect(updateUserProfile('user-4', {})).rejects.toThrow('Update failed');
    });
});
...
describe('saveResume', () => {
    ...
    it('rethrows errors', async () => {
        ...
        await expect(saveResume('user-5', 'text', 'file.pdf')).rejects.toThrow('Add failed');
    });
});
...
describe('saveJobDescription', () => {
    ...
    it('rethrows errors', async () => {
        ...
        await expect(saveJobDescription('user-7', 'desc')).rejects.toThrow('JD save failed');
    });
});
...
describe('saveMessage', () => {
    ...
    it('does NOT throw even if addDoc fails (non-blocking)', async () => {
        ...
        await expect(saveMessage('user-9', 'int-1', { text: 'hi' })).resolves.not.toThrow();
    });
});
...
describe('saveFullResumeAnalysis', () => {
    it('calls setDoc twice: once for resumeAnalysis and once for prepContext', async () => {
        ...
        await saveFullResumeAnalysis('user-11', 'resume-1', 'cv.pdf', 'jd-1', analysis);
        expect(mockSetDoc).toHaveBeenCalledTimes(2);
    });

    it('rethrows errors', async () => {
        mockSetDoc.mockRejectedValue(new Error('Write failed'));
        await expect(saveFullResumeAnalysis('user-11', 'r1', 'cv.pdf', 'jd1', {})).rejects.toThrow('Write failed');
    });
});
...
describe('getResumeAnalysisWithData', () => {
    ...
    it('hydrates resume and JD data when both IDs are present', async () => {
        const analysisData = { resumeId: 'r1', jobDescriptionId: 'jd1', analysis: {} };
        mockGetDoc
            .mockResolvedValueOnce({ exists: () => true, data: () => analysisData }) // analysis doc
            .mockResolvedValueOnce({ exists: () => true, data: () => ({ text: 'resume body', fileName: 'cv.pdf' }) }) // resume
            .mockResolvedValueOnce({ exists: () => true, data: () => ({ text: 'Job description text' }) }); // JD
    ...
    it('returns empty strings when resume and JD are missing', async () => {
        const analysisData = { resumeId: null, jobDescriptionId: null, analysis: {} };
        mockGetDoc.mockResolvedValueOnce({ exists: () => true, data: () => analysisData });
...
describe('processInterviewFeedback', () => {
    it('calls updateDoc with completed status', async () => {
        ...
        await processInterviewFeedback('user-13', 'session-1', feedback, 'Engineer');
    ...
    it('creates a recommendation when score < 70 and weaknesses exist', async () => {
        ...
        await processInterviewFeedback('user-13', 'session-2', feedback, 'Engineer');
    ...
    it('does NOT create recommendation when score >= 70', async () => {
        ...
        await processInterviewFeedback('user-13', 'session-3', feedback, 'Engineer');
    ...
    it('does NOT create recommendation when no weaknesses exist (even if score < 70)', async () => {
        ...
        await processInterviewFeedback('user-13', 'session-4', feedback, 'Engineer');
    ...
    it('rethrows errors from updateDoc', async () => {
        mockUpdateDoc.mockRejectedValue(new Error('Update failed'));
        await expect(
            processInterviewFeedback('user-13', 'session-5', { performanceScore: 80, weaknesses: [] }, 'Dev')
