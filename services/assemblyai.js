...
export class AssemblyAISession {
  ...
  async connect() {
    try {
      // Get a short-lived, session-scoped token from our API route.
      // The permanent AssemblyAI API key never reaches the browser.
      const response = await authedFetch("/api/assemblyai-token", {
        method: "POST",
      });
      ...
      // Connect to AssemblyAI Real-Time WebSocket with the temporary token
...
export async function createAssemblyAISession(config) {
