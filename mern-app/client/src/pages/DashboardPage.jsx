...
import { listInterviews } from '../features/interview/services/interviewApi';
...
export default function DashboardPage() {
  ...
  const recommendation = user?.recommendation?.message ? user.recommendation : null;
  const prepContext = user?.prepContext?.targetRole ? user.prepContext : null;
    ...
    const fetchData = async () => {
      try {
        ...
        const [history] = await Promise.all([
          listInterviews(5),
          // Refresh recommendation / prep context alongside the history.
          refreshUser().catch(() => {}),
        ]);
      ...
      <header className="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">
            {getTimeGreeting()},{' '}
            <span className="text-indigo-600">{user?.name?.split(' ')[0] || 'Candidate'}</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 animate-slide-up">
        ...
        >
          ...
          <p className="text-slate-500 mb-8 leading-relaxed relative z-10">
            See how your resume matches a real job description and uncover exactly what
            interviewers will focus on.
          </p>
        ...
        >
          ...
          <p className="text-slate-500 mb-8 leading-relaxed relative z-10">
            Simulate a real technical interview with our AI voice assistant. Get grilled on your
            stack and receive instant feedback.
          </p>
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-slate-50 to-white rounded-3xl border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden">
          {recommendation ? (
            ...
            <div className="relative z-10">
              ...
              <button
                onClick={() => navigate(recommendation.targetAction || '/setup')}
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-500 mb-6 max-w-lg leading-relaxed">
                Analyze your resume to let our AI identify your skill gaps and tailor a personal
                plan.
              </p>
              <button
                onClick={() => navigate('/resume-analyzer')}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-amber-50/80 p-5 rounded-2xl border border-amber-100/50">
            <p className="text-amber-800 text-xs font-semibold leading-relaxed flex gap-2">
              ...
              <span>
                Users who practice <span className="font-bold underline">6+ interviews</span>{' '}
            ...
            <ul className="divide-y divide-slate-100">
              {interviews.map((session) => (
                <li key={session.id} className="group hover:bg-slate-50 transition-colors duration-200">
                  <Link to={`/result/${session.id}`} className="block p-6">
                            ...
                            <span
                              className={`text-lg font-bold ${
                                (session.feedback?.performanceScore || 0) >= 80
                                  ? 'text-emerald-600'
                                  : (session.feedback?.performanceScore || 0) >= 60
                                    ? 'text-amber-600'
                                    : 'text-slate-700'
                              }`}
                            >
