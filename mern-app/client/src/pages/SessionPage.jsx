...
import { InterviewMode } from '../constants';
import { useToastError } from '../hooks/useToastError';
import { createLiveClient } from '../features/interview/services/geminiLive';
import { AssemblyAISession } from '../features/interview/services/assemblyai';
import { generateInterviewerPersona } from '../features/interview/services/persona';
import {
  getInterview,
  savePersona,
  saveMessage,
...
export default function SessionPage() {
  ...
  const [, setIsMicOn] = useState(false);
          ...
          sessionPromiseRef.current
            .then((liveSession) => {
              liveSession
                ...
                .catch((e) => console.error('Failed to force turn completion:', e));
            })
            .catch(() => {});
        }
      }
  ...
  const initSession = async () => {
    ...
    try {
      ...
      if (data.config.jobDescription && !data.interviewerPersona && !personaGeneratedRef.current) {
        ...
        setIsLoading(true);
        try {
          const persona = generateInterviewerPersona(data.config);
      ...
      if (error?.response?.status === 404) {
        showError({ message: 'Session not found.', severity: 'error' });
        navigate('/dashboard');
      } else {
        showError({
    ...
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        const msg = isSecure
          ...
          : `Microphone blocked. You are on an insecure connection (${window.location.hostname}). Please use localhost in the address bar.`;

        showError({ message: msg, severity: 'error' });
        setConnected(false);
          ...
          callbacks: {
            onopen: async () => {
              ...
              try {
                ...
                // Increased buffer size to reduce main thread load (lag)
            ...
            onmessage: async (message) => {
              ...
              if (message.serverContent?.turnComplete) {
                ...
                const userText =
                  geminiUserText ||
                  (!geminiGotInputTranscriptRef.current && asmUserText ? asmUserText : '');
                if (!geminiUserText && asmUserText)
                  console.log('[AssemblyAI Fallback] Using fallback transcript:', asmUserText);
                ...
                if (userText || modelText) {
                  ...
                  setMessages((prev) => {
                    ...
                    if (id) {
                      // Background save
                      newMsgs.forEach((msg) => saveMessage(id, msg));
                    }
                    return updated;
                  });
  ...
  const finishInterview = useCallback(async () => {
    ...
    try {
      ...
      await generateFeedback(id, msgsToAnalyze);
      navigate(`/result/${id}`);
    } catch (error) {
      console.error(error);
      showError({
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            {/* Persona is stored as system prompt internally, not displayed */}
            ...
            <p className="text-xs text-slate-500 font-medium">{session?.config.role} Position</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          {/* Status Indicators */}
          ...
          {connected || isReconnecting ? (
                ...
                {realtimeTranscript ? (
                  <p className="text-lg font-medium text-white/90 animate-fade-in leading-relaxed">
                    <span className="opacity-50 text-sm uppercase block mb-2 tracking-widest">
                      {realtimeTranscript.role === 'user' ? 'You' : 'AI Interviewer'}
                    </span>
                    "{realtimeTranscript.text}"
                  </p>
                ) : (
                  <p className="text-white/30 text-sm font-mono">
                    {isReconnecting ? 'Reestablishing connection...' : 'Listening...'}
                  </p>
                )}
              </div>
            ...
            <div className="text-center z-10">
              ...
              <p className="text-indigo-200 mb-8 max-w-sm mx-auto">
                Click below to connect to the AI interviewer. Please ensure you are in a quiet
                environment.
              </p>
              <button
