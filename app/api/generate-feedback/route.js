...
function validateBody({ transcript, config }) {
  if (!transcript || !config) {
    throw badRequest('Transcript and config are required');
  }
  if (!Array.isArray(transcript) || transcript.length === 0) {
    throw badRequest('Transcript must be a non-empty array');
  }
  if (transcript.length > MAX_TRANSCRIPT_MESSAGES) {
    throw badRequest('Transcript is too long');
  }
  for (const message of transcript) {
    if (!message || typeof message !== 'object') {
      throw badRequest('Each transcript entry must be an object');
    }
    if (message.text !== undefined && typeof message.text !== 'string') {
      throw badRequest('Transcript entry text must be a string');
    }
    if (message.text && message.text.length > MAX_MESSAGE_CHARS) {
      throw badRequest('Transcript entry is too long');
    }
  }
...
export const POST = createApiHandler(
  'generate-feedback',
  async ({ body }) => {
    const { transcript, config, resumeText, jobDescription } = body;

    validateBody({ transcript, config });
      ...
      try {
        ...
        if (!feedback.categoryScores) {
          ...
          feedback.categoryScores = {
            ...
            cvAlignment: hasResume
              ? Math.min(10, Math.max(0, Math.round((base - 3) / 10)))
              : 0,
          };
        }
        ...
        if (!feedback.overallGrade) {
          ...
          feedback.overallGrade =
            s >= 90 ? 'A' : s >= 85 ? 'A-' : s >= 80 ? 'B+' : s >= 75 ? 'B' : s >= 70 ? 'B-' : s >= 65 ? 'C+' : s >= 60 ? 'C' : s >= 55 ? 'C-' : 'D';
        }
        ...
        if (!feedback.hiringDecision) {
          ...
          feedback.hiringDecision =
            s >= 85 ? 'Strong Hire' : s >= 70 ? 'Hire' : s >= 55 ? 'Borderline' : 'No Hire';
        }
