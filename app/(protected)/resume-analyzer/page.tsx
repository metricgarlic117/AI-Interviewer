...
import { ResumeAnalysis, InterviewLevel } from '@/types';
import { saveFullResumeAnalysis, saveResume, saveJobDescription } from '@/services/userData';
...
type Step = 'upload' | 'jd' | 'validation' | 'analysis' | 'results';
...
export default function ResumeAnalyzerPage() {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (text.length < 50) {
        throw new Error("Resume appears empty or is an image-based PDF. Please use a text-based PDF.");
      }
      setResumeText(text);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const jdId = jdText.length > 50 ? await saveJobDescription(user.uid, jdText) : undefined;
  ...
  const startMockInterview = () => {
    ...
    if (sen.includes('senior') || sen.includes('lead')) level = InterviewLevel.Senior;
    ...
    router.push(`/setup?fromAnalysis=true&role=${encodeURIComponent(role)}&level=${level}&techStack=${encodeURIComponent(techStack)}&jd=${encodeURIComponent(jdText)}&resume=${encodeURIComponent(resumeText)}`);
  };
  ...
  const renderProgress = () => {
    ...
    const activeIdx = ['upload', 'jd', 'validation', 'results'].indexOf(currentStep);
        ...
        <div className="grid grid-cols-4 w-full relative">
          {steps.map((label, idx) => (
            <div key={idx} className="flex flex-col items-center gap-2 relative z-10">
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 border-4 ${idx <= visualIdx
                ? 'bg-indigo-600 text-white border-white shadow-lg shadow-indigo-200'
                : 'bg-white text-slate-500 border-slate-200'
                }`}>
                {idx < visualIdx ? <i className="fa-solid fa-check"></i> : idx + 1}
              </div>
              <span className={`text-xs font-medium text-center ${idx <= visualIdx ? 'text-indigo-600' : 'text-slate-400'}`}>{label}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (currentStep === 'analysis') {
    return (
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center bg-slate-50">
        ...
        <h2 className="text-2xl font-bold text-slate-800 animate-pulse">Analyzing Resume Fit...</h2>
        <p className="text-slate-500 mt-2">Comparing skills, seniority, and keywords against the job description.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-slate-900">AI Resume Analyzer</h1>
        <p className="text-slate-500 mt-2">Optimize your resume for the specific role you want.</p>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden min-h-[400px] flex flex-col relative">
        {error && (
          <div className="absolute top-4 left-4 right-4 z-10 bg-red-50 text-red-600 p-3 rounded-xl text-sm border border-red-100 flex items-center shadow-sm animate-fade-in">
            ...
            <button onClick={() => setError(null)} className="ml-auto hover:text-red-800"><i className="fa-solid fa-xmark"></i></button>
          </div>
        )}
        ...
        {currentStep === 'upload' && (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center animate-slide-up">
            ...
            <p className="text-slate-500 mb-8 max-w-sm">We accept PDF files. We'll scan it to extract your skills and experience.</p>
            ...
            <label className="group relative inline-flex flex-col items-center justify-center w-full max-w-lg h-48 border-2 border-slate-300 border-dashed rounded-2xl cursor-pointer hover:bg-slate-50 hover:border-indigo-400 transition-all">
              ...
              <input type="file" className="hidden" accept="application/pdf" onChange={handleResumeUpload} disabled={loading} />
            </label>
          </div>
        )}
        ...
        {currentStep === 'jd' && (
          <div className="flex-1 flex flex-col p-8 animate-slide-up">
              ...
              <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-xl hover:bg-slate-50 transition-all">
                {loading ? (
                  ...
                  <label className="cursor-pointer text-center p-10 w-full h-full flex flex-col items-center justify-center">
                    ...
                    <p className="text-sm font-bold text-slate-700">Upload PDF or Screenshot</p>
                    <p className="text-xs text-slate-500 mt-1">We'll extract the text automatically.</p>
                    <input type="file" className="hidden" accept="application/pdf, image/*" onChange={handleJdFileUpload} />
                  </label>
                )}
              </div>
            )}
            ...
            <div className="flex justify-between mt-6">
              <button onClick={() => setCurrentStep('upload')} className="text-slate-500 hover:text-slate-800 text-sm font-bold px-4">Back</button>
              <button
        ...
        {currentStep === 'validation' && (
          <div className="flex-1 flex flex-col p-8 animate-slide-up">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Review Data</h2>
            ...
            <div className="space-y-4 mb-8">
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-bold text-slate-800 text-sm">Resume Parsed</h4>
                  <p className="text-xs text-slate-500 mt-1 line-clamp-2">{resumeText.substring(0, 150)}...</p>
                  <div className="mt-2 text-xs font-bold text-indigo-600 bg-indigo-50 inline-block px-2 py-1 rounded">
                    {resumeFileName}
                  </div>
                </div>
                <button onClick={() => setCurrentStep('upload')} className="text-xs text-slate-400 hover:text-indigo-600 underline">Edit</button>
              </div>
              ...
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-bold text-slate-800 text-sm">Job Description Ready</h4>
                  <p className="text-xs text-slate-500 mt-1 line-clamp-2">{jdText.substring(0, 150)}...</p>
                  <div className="mt-2 text-xs font-bold text-slate-500">
                    {jdText.length} characters detected
                  </div>
                </div>
                <button onClick={() => setCurrentStep('jd')} className="text-xs text-slate-400 hover:text-indigo-600 underline">Edit</button>
              </div>
            </div>

            <div className="mt-auto flex justify-between items-center">
              <button onClick={() => setCurrentStep('jd')} className="text-slate-500 hover:text-slate-800 text-sm font-bold px-4">Back</button>
              <button
                onClick={runAnalysis}
          ...
          <div className="flex-1 flex flex-col p-8 animate-slide-up overflow-y-auto max-h-[600px] scrollbar-hide">
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-8 items-center bg-gradient-to-r from-indigo-600 to-violet-600 rounded-2xl p-6 text-white shadow-lg">
              <div className="relative">
                <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 96 96">
                  <circle cx="48" cy="48" r="36" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-indigo-900/30" />
                  <circle cx="48" cy="48" r="36" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={226.2} strokeDashoffset={226.2 - (226.2 * analysisResult.matchScore) / 100} strokeLinecap="round" className="text-white transition-all duration-1000 ease-out" />
                </svg>
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-indigo-100 text-sm leading-relaxed">{analysisResult.roleFitSummary}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><i className="fa-solid fa-layer-group text-indigo-500"></i> Skills Analysis</h3>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {analysisResult.skillsGap.matchingSkills.map((s, i) => (
                        <span key={i} className="px-2 py-1 bg-green-50 text-green-700 text-xs rounded-md border border-green-100">{s}</span>
                      ))}
                    </div>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-red-500 uppercase mb-2">Missing / Keywords to Add</p>
                    <div className="flex flex-wrap gap-2">
                      {analysisResult.skillsGap.missingSkills.map((s, i) => (
                        <span key={i} className="px-2 py-1 bg-red-50 text-red-600 text-xs rounded-md border border-red-100">{s}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {/* Improvements */}
              <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><i className="fa-solid fa-pen-nib text-amber-500"></i> Resume Improvements</h3>
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-6 mb-8">
              <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><i className="fa-solid fa-comments text-indigo-500"></i> Likely Interview Topics</h3>
            ...
            <div className="mt-auto flex flex-col sm:flex-row gap-4 justify-center pt-6 border-t border-slate-100">
              ...
              >
                <i className="fa-solid fa-microphone-lines"></i> Practice Interview for this Role
              </button>
              <button
