...
export default function ProfilePage() {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      if (error.code === "auth/requires-recent-login") {
        setMessage({
          type: "error",
          text: "Please logout and login again to change your password.",
        });
      } else {
        setMessage({
          type: "error",
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12 animate-slide-up">
      ...
      {message && (
        <div
          className={`p-4 mb-6 rounded-xl flex items-center gap-3 ${message.type === "success" ? "bg-green-50 text-green-700 border border-green-100" : "bg-red-50 text-red-700 border border-red-100"}`}
        >
          <i
            className={`fa-solid ${message.type === "success" ? "fa-check-circle" : "fa-circle-exclamation"}`}
          ></i>
          {message.text}
        </div>
      )}
