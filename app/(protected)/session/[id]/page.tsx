...
import { ai, generateInterviewFeedback, generateInterviewerPersona } from '@/services/gemini';
import { AssemblyAISession } from '@/services/assemblyai';
import { processInterviewFeedback, getResume, getJobDescription, saveMessage, getSessionMessages } from '@/services/userData';
import { InterviewSession as SessionType, Message, InterviewMode, ErrorCode, ErrorSeverity } from '@/types';
...
export default function SessionPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [realtimeTranscript, setRealtimeTranscript] = useState<{ role: 'user' | 'model', text: string } | null>(null);
    ...
    const sessionDataRef = useRef<SessionType | null>(null);
    ...
    const assemblyAISessionRef = useRef<AssemblyAISession | null>(null);
    const assemblyAITranscriptRef = useRef<string>('');       // accumulates fallback text per turn
    const geminiGotInputTranscriptRef = useRef<boolean>(false); // tracks if Gemini delivered input transcript this turn
    ...
    useEffect(() => {
        if (connected) {
            ...
            silenceTimerRef.current = setInterval(() => {
                ...
                if (timeSinceSpeech > 180000) { // 3 mins silence
                        ...
                        sessionPromiseRef.current.then((session) => {
                            (session as any).sendRealtimeInput({
                                clientContent: {
                                    turns: [{
                                        role: 'user',
                                        parts: [{ text: "I haven't said anything for 3 minutes. Please prompt me." }]
                                    }],
                                    turnComplete: true
                                }
                            }).catch((e: any) => console.error("Failed to send nudge:", e));
                        }).catch(() => { });
                        lastSpeechTimeRef.current = Date.now();
                    }
                }
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.code === 'Space' && !e.repeat && connected && !isSpacebarHeldRef.current) {
                e.preventDefault();
            ...
            if (e.code === 'Space' && isSpacebarHeldRef.current) {
                ...
                if (sessionPromiseRef.current) {
                    sessionPromiseRef.current.then((session) => {
                        ...
                        }).catch((e: any) => console.error('Failed to force turn completion:', e));
                    }).catch(() => { });
                }
            }
        };
    ...
    const initSession = async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                const data = { id: docSnap.id, ...docSnap.data() } as SessionType;
                ...
                if (data.config.resumeId && !data.config.resumeText) {
                    ...
                    } catch (e) { console.error("Failed to load resume content", e); }
                }
                ...
                if (data.config.jdId && !data.config.jobDescription) {
                    ...
                    } catch (e) { console.error("Failed to load JD content", e); }
                }

                // Generate Persona (Safe Call)
                if (data.config.jobDescription && !data.interviewerPersona && !personaGeneratedRef.current) {
                ...
                const allMessages = [...legacyMessages, ...subCollMessages as any[]].sort((a, b) => a.timestamp - b.timestamp);
    ...
    const disconnectSession = (fullCleanup = false) => {
        ...
        try {
            if (inputAudioContextRef.current && inputAudioContextRef.current.state !== 'closed') {
                inputAudioContextRef.current.close().catch(e => console.warn('Input AC close fail', e));
            }
        ...
        try {
            if (outputAudioContextRef.current && outputAudioContextRef.current.state !== 'closed') {
                outputAudioContextRef.current.close().catch(e => console.warn('Output AC close fail', e));
            }
        ...
        if (assemblyAISessionRef.current) {
            try { assemblyAISessionRef.current.disconnect(); } catch (e) { }
      ...
      `;
        ...
        try {
            ...
            try {
                ...
                let msg = "Microphone access denied. Please check your browser settings.";
                    ...
                    onopen: async () => {
                        ...
                        try {
                            ...
                            if (inputAudioContextRef.current.state === 'suspended') {
                                try {
                                    ...
                                    console.warn('Failed to resume input AudioContext in onopen:', e);
                                }
                            }
                            ...
                            if (!stream.active && stream.getAudioTracks().length === 0) {
                                throw new Error('MediaStream is inactive or has no audio tracks');
                            }
                            ...
                            processor.onaudioprocess = (e) => {
                                ...
                                // VAD for visualizer
                                let sum = 0;
                                for (let i = 0; i < downsampledData.length; i++) { sum += downsampledData[i] * downsampledData[i]; }
                                const rms = Math.sqrt(sum / downsampledData.length);
                                setVisualizerLevel(Math.min(1, rms * 5));

                                if (rms > 0.02) { lastSpeechTimeRef.current = Date.now(); }
                                ...
                                if (isSpacebarHeldRef.current) {
                                    ...
                                    if (sessionPromiseRef.current) {
                                        sessionPromiseRef.current.then((session) => {
                                            session.sendRealtimeInput({ media: pcmBlob }).catch((e: any) => console.warn("Send audio fail", e));
                                        }).catch(() => { });
                                    }
                        ...
                        try {
                            const asmSession = new AssemblyAISession({
                                ...
                                onError: (err) => console.warn('[AssemblyAI Fallback] Error:', err),
                                onClose: () => console.log('[AssemblyAI Fallback] Closed'),
                                sampleRate: 16000,
                            });
                            await asmSession.connect();
                            assemblyAISessionRef.current = asmSession;
                            console.log('[AssemblyAI Fallback] Connected and ready as secondary transcription provider');
                        } catch (e) {
                            console.warn('[AssemblyAI Fallback] Failed to connect (non-fatal — Gemini will handle transcription):', e);
                        }
                    },
                    onmessage: async (message: LiveServerMessage) => {
                        ...
                        if (base64Audio) {
                            try {
                                ...
                                const nextTime = Math.max(outputCtx.currentTime + 0.02, nextStartTimeRef.current);
                        ...
                        if (message.serverContent?.outputTranscription) {
                            ...
                            if (text) {
                                ...
                                if (text) {
                                    ...
                                    setRealtimeTranscript({ role: 'model', text: currentOutputTranscriptionRef.current });
                                }
                            }
                        }
                        ...
                        if (message.serverContent?.inputTranscription) {
                            ...
                            if (text) {
                                ...
                                if (text) {
                                    ...
                                    setRealtimeTranscript({ role: 'user', text: currentInputTranscriptionRef.current });
                                    lastSpeechTimeRef.current = Date.now();
                                }
                            }
                        }
                        ...
                        if (message.serverContent?.turnComplete) {
                            const geminiUserText = currentInputTranscriptionRef.current.trim();
                            ...
                            const userText = geminiUserText || (!geminiGotInputTranscriptRef.current && asmUserText ? asmUserText : '');
                            if (!geminiUserText && asmUserText) console.log('[AssemblyAI Fallback] Using fallback transcript:', asmUserText);
                            const modelText = currentOutputTranscriptionRef.current.trim();
                            if (userText || modelText) {
                                const newMsgs: Message[] = [];
                                if (userText) newMsgs.push({ role: 'user', text: userText, timestamp: Date.now() });
                                if (modelText) newMsgs.push({ role: 'model', text: modelText, timestamp: Date.now() });

                                setMessages(prev => {
                                    const updated = [...prev, ...newMsgs];
                                    if (id && auth.currentUser) {
                                        // Background Save (Subcollection)
                                        newMsgs.forEach(msg => saveMessage(auth.currentUser!.uid, id, msg));
                                    }
                                    return updated;
                                });
    ...
    const handleConnectionLoss = () => {
        ...
        if (retryCountRef.current < 3) {
            ...
            // Exponential Backoff
            setTimeout(() => { connectToGemini(); }, 1000 * retryCountRef.current);
        } else {
            disconnectSession(true);
            showError({
    ...
    const finishInterview = useCallback(async () => {
        ...
        try {
            ...
            await processInterviewFeedback(user.uid, id, feedback, sessionDataRef.current.config.role);
            router.push(`/result/${id}`);
    ...
    if (analyzing) {
        return (
            <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/90 backdrop-blur-md">
                ...
                <h2 className="text-2xl font-bold text-slate-800 mt-6">Analyzing Performance</h2>
                <p className="text-slate-500 mt-2">Generating detailed feedback report...</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] max-w-5xl mx-auto px-4 py-6">
            ...
            <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h2 className="font-bold text-slate-900 flex items-center gap-2">
                            AI Interviewer
                            <span className="text-xs font-normal text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">Interviewer</span>
                        </h2>
                ...
                <div className="flex items-center gap-4">
                    {/* Status Indicators */}
                    <div className={`hidden sm:flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border ${session?.config.mode === InterviewMode.Coach ? 'bg-green-50 text-green-700 border-green-200' :
                        session?.config.mode === InterviewMode.Stress ? 'bg-red-50 text-red-700 border-red-200' :
                    ...
                    <button onClick={finishInterview} className="px-4 py-2 bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 rounded-lg text-sm font-bold transition-colors">
                        End Call
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex gap-6 overflow-hidden">
                    ...
                    {connected || isReconnecting ? (
                        <div className="relative z-10 flex flex-col items-center">
                            ...
                            <div className="relative w-40 h-40 mb-10">
                                <div className={`absolute inset-0 bg-indigo-500 rounded-full blur-3xl opacity-30 ${connected ? 'animate-pulse-slow' : ''}`}></div>
                                            ...
                                            <div className="flex items-center gap-1 h-12">
                                                {[...Array(5)].map((_, i) => (
                                                    <div
                                                        ...
                                                        style={{ height: `${20 + Math.random() * (visualizerLevel * 80)}%` }}
                                                    ></div>
                                                ))}
                                            </div>
                                        )}
                            ...
                            <div className="max-w-xl text-center min-h-[80px]">
                                {realtimeTranscript ? (
                                    <p className="text-lg font-medium text-white/90 animate-fade-in leading-relaxed">
                                        <span className="opacity-50 text-sm uppercase block mb-2 tracking-widest">{realtimeTranscript.role === 'user' ? 'You' : 'AI Interviewer'}</span>
                                        "{realtimeTranscript.text}"
                                    </p>
                                ) : (
                                    <p className="text-white/30 text-sm font-mono">{isReconnecting ? 'Reestablishing connection...' : 'Listening...'}</p>
                                )}
                            </div>
                            ...
                            <div className="mt-8">
                                {isPushToTalkActive ? (
                                    ...
                                    <div className="flex items-center gap-3 px-6 py-3 bg-white/10 border-2 border-white/20 rounded-full backdrop-blur-sm">
                                        <kbd className="px-2 py-1 bg-white/20 rounded text-xs font-mono font-bold text-white">SPACE</kbd>
                                        <span className="text-white/70 font-medium text-sm">Hold to speak</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="text-center z-10">
                            ...
                            <p className="text-indigo-200 mb-8 max-w-sm mx-auto">Click below to connect to the AI interviewer. Please ensure you are in a quiet environment.</p>
                            <button
                ...
                <div className="hidden lg:flex w-96 bg-white rounded-3xl border border-slate-200 flex-col shadow-lg overflow-hidden">
                    <div className="p-4 border-b border-slate-100 bg-slate-50">
                        <h3 className="font-bold text-slate-700 text-sm uppercase tracking-wide">Transcript</h3>
                    </div>
